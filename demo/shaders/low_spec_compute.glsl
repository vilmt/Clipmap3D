...
void main() {
	...
	// material parameters
	float height = h;
	vec3 normal = normalize(vec3(-gradient.x * params.height_amplitude, 1.0, -gradient.y * params.height_amplitude));
	...
	float w_grass = smoothstep(0.5, 0.49, height) * smoothstep(0.25, 0.24, slope);
	brush_add(mat, GRASS_ID, w_grass); // paint grass in flat, low areas
	
	float w_snow = smoothstep(0.6, 0.62, height) * smoothstep(0.31, 0.25, slope);
