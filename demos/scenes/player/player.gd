...
# If these are too close to 90.0, may cause rendering issues
...
func _unhandled_input(event: InputEvent):
	if event is InputEventMouseMotion:
		...
		var motion: Vector2 = -0.001 * mouse_event.relative * sensitivity
		
		rotation.y += motion.x
		orthonormalize() # prevents floating point error accumulation
		spring_arm.rotation.x = clampf(spring_arm.rotation.x + motion.y, MIN_PITCH, MAX_PITCH)
